        ...
        **kwargs,
        ...
        if image.dim() == 5:
            ...
            if images_racd is not None:
                assert isinstance(images_racd, list) or isinstance(images_racd, tuple)
                image_embeds_racd_l = []
                image_atts_racd_l = []
                for image_nn in images_racd:
                    with self.maybe_autocast():
                        image_embeds_nn = self.ln_vision(self.visual_encoder(image_nn))
                    image_atts_nn = torch.ones(image_embeds_nn.size()[:-1], dtype=torch.long).to(image.device)
                    image_embeds_racd_l.append(image_embeds_nn)
                    image_atts_racd_l.append(image_atts_nn)
            ...
            if self.qformer_text_input:
                ...
                if images_racd is not None:
                    query_output_racd_l = []
                    for image_embeds_nn, image_atts_nn in zip(image_embeds_racd_l, image_atts_racd_l):
                        query_output_nn = self.Qformer.bert(
                            text_Qformer.input_ids,
                            attention_mask=Qformer_atts,
                            query_embeds=query_tokens,
                            encoder_hidden_states=image_embeds_nn,
                            encoder_attention_mask=image_atts_nn,
                            return_dict=True,
                            )
                        query_output_racd_l.append(query_output_nn)
                ...
                if images_cd is not None or images_racd is not None:
            ...
            if images_racd is not None:
                inputs_llm_racd_l, atts_llm_racd_l = [], []
                for query_output_nn in query_output_racd_l:
                    inputs_llm_nn = self.llm_proj(query_output_nn.last_hidden_state[:,:query_tokens.size(1),:])
                    atts_llm_nn = torch.ones(inputs_llm_nn.size()[:-1], dtype=torch.long).to(image.device)
                    inputs_llm_racd_l.append(inputs_llm_nn)
                    atts_llm_racd_l.append(atts_llm_nn)
        ...
        input_ids = llm_tokens.input_ids.clone()
        llm_tokens_attention_mask = llm_tokens.attention_mask.clone()
        if extra_input_ids is not None:
            # for visual hallucination analysis
            input_ids = torch.cat([input_ids, extra_input_ids], dim=1)
            extra_attention_mask = torch.ones_like(extra_input_ids).to(input_ids.device)
            llm_tokens_attention_mask = torch.cat([llm_tokens_attention_mask, extra_attention_mask], dim=1)
        ...
        with self.maybe_autocast():
            ...
            inputs_embeds = torch.cat([inputs_llm, inputs_embeds], dim=1)
            # attention_mask = torch.cat([atts_llm, llm_tokens.attention_mask], dim=1)
            attention_mask = torch.cat([atts_llm, llm_tokens_attention_mask], dim=1)
            ...
            if images_racd is not None:
                ...
                inputs_embeds_nn = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
                for inputs_llm_nn, atts_llm_nn in zip(inputs_llm_racd_l, atts_llm_racd_l):
                    inputs_embeds_racd = torch.cat([inputs_llm_nn, inputs_embeds_nn], dim=1)
                    attention_mask_nn = torch.cat([atts_llm_nn, llm_tokens.attention_mask], dim=1)
                    inputs_embeds_racd_l.append(inputs_embeds_racd)
            ...
            model_outputs = self.llm_model.generate(
                ...
                images_racd=inputs_embeds_racd_l if images_racd is not None else None,
                ...
                return_dict_in_generate=kwargs.get("return_dict_in_generate", None),
        ...
        if return_dict_in_generate:
            # model_outputs.scores: [num_new_tokens, 2, 1]
            outputs = model_outputs.sequences  # [1, [0,num_new_tokens, 2, 1]]
            output_ids = outputs.clone()  # remove pad, start from new tokens
            outputs[outputs == 0] = 2 # convert output id 0 to 2 (eos_token_id)
            output_text = self.llm_tokenizer.batch_decode(outputs, skip_special_tokens=True)
            output_text = [text.strip() for text in output_text]  # sentence composed of num_new_tokens, list_len=1
            return output_text, model_outputs.scores, output_ids
        else:
