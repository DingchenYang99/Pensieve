...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        images_racd: Optional[List[torch.FloatTensor]] = None,
    ...
    def prepare_inputs_for_generation_racd(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs
    ):
        if past_key_values:
            input_ids = input_ids[:, -1:]
        ...
        # if `inputs_embeds` are passed, we only want to use them in the 1st generation step
        j = kwargs.get("nn_idx", None)
        if inputs_embeds is not None and past_key_values is None:
            if j is not None:
                model_inputs = {"inputs_embeds": kwargs.get("images_racd")[j]}
            else:
                model_inputs = {"inputs_embeds": kwargs.get("images_racd")}
        else:
            model_inputs = {"input_ids": input_ids}
