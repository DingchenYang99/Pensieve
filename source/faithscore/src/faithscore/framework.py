...
from src.faithscore.llava15 import LLaVA
from src.faithscore.llama_pre import load_llama, stage1_llama
from src.faithscore.utils import llava15, ofa
...
class FaithScore():
    ...
    def stage2(self, labeld_sub_sen, ):
        ...
        for i in range(len(Entities)):
            try:
                hallucinations.append(Entities[i] + Relations[i] + Colors[i] + Counting[i] + Others[i])
            except:
                print(f'{i} failed')
                continue
    ...
    def faithscore(self, answers, images, labeld_sub_sen=None, atomic_facts=None, calculate_sentence_score=False):
        ## Stage 1: Sub-setence Identification
        if labeld_sub_sen is None:
        ...
        # print(fact_scores)
        if calculate_sentence_score:
            # assert atomic_facts is None
            atomic_facts, Entities, Relations, Colors, Counting, Others = self.stage2(labeld_sub_sen)
            score, fact_scores = self.stage3(atomic_facts, images)
            ...
            return score, sentence_score, atomic_facts, fact_scores
        else:
            if atomic_facts is not None:
                ...
                atomic_facts, Entities, Relations, Colors, Counting, Others = self.stage2(labeld_sub_sen)
                score, fact_scores = self.stage3(atomic_facts, images)
            return score, atomic_facts, fact_scores
